...
class Student extends CI_Controller
{
    ...
    function pay($gateway = '', $invoice_id = '') {
      ...
      if ($gateway == 'stripe') {
            ...
            if ($payment_success == true) {
                $this->session->set_flashdata('flash_message', get_phrase('payment_successfull'));
                redirect(base_url().'student/invoice/'.$student_id, 'refresh');
            } else {
        ...
        if ($param1 == 'send_new') {
            ...
            if ($_FILES['attached_file_on_messaging']['name'] != "") {
              if($_FILES['attached_file_on_messaging']['size'] > $max_size){
                ...
                redirect(base_url() . 'student/message/message_new/', 'refresh');
              }
              else{
    ...
    function group_message($param1 = "group_message_home", $param2 = ""){
      ...
      if ($param1 == 'group_message_read') {
        ...
        if ($_FILES['attached_file_on_messaging']['name'] != "") {
          if($_FILES['attached_file_on_messaging']['size'] > $max_size){
            $this->session->set_flashdata('error_message' , get_phrase('file_size_can_not_be_larger_that_2_Megabyte'));
            redirect(base_url() . 'student/group_message/group_message_read/' . $param2, 'refresh');
          }
          else{
    ...
    function book_request($param1 = "", $param2 = "")
    {
        ...
        if ($param1 == "create")
        {
            ...
            redirect(base_url() . 'student/book_request', 'refresh');
        }

...
class Admin extends CI_Controller
{
  ...
  function student_profile($student_id)
  {
    ...
    function delete_student($student_id = '', $class_id = '') {
      ...
      redirect(base_url() . 'admin/student_information/' . $class_id, 'refresh');
    }
    ...
    function marks2($exam_id = '', $class_id = '', $subject_id = '')
    {
        ...
        if ($this->input->post('operation') == 'selection') {
            ...
            if ($page_data['exam_id'] > 0 && $page_data['class_id'] > 0 && $page_data['subject_id'] > 0) {
                redirect(base_url() . 'admin/marks2/' . $page_data['exam_id'] . '/' . $page_data['class_id'] . '/' . $page_data['subject_id'], 'refresh');
            } else {
                $this->session->set_flashdata('mark_message', 'Choose exam, class and subject');
                redirect(base_url() . 'admin/marks2/', 'refresh');
            }
        }
        if ($this->input->post('operation') == 'update') {
            ...
            $this->session->set_flashdata('flash_message' , get_phrase('data_updated'));
            redirect(base_url() . 'admin/marks2/' . $this->input->post('exam_id') . '/' . $this->input->post('class_id') . '/' . $this->input->post('subject_id'), 'refresh');
        }
        $page_data['exam_id']    = $exam_id;
        $page_data['class_id']   = $class_id;
    ...
    function marks_selector()
    {
        ...
        redirect(base_url() . 'admin/marks_manage_view/' . $data['exam_id'] . '/' . $data['class_id'] . '/' . $data['section_id'] . '/' . $data['subject_id'] , 'refresh');
    }
    else{
        $this->session->set_flashdata('error_message' , get_phrase('select_all_the_fields'));
        $page_data['page_name']  =   'marks_manage';
    ...
    function marks_update($exam_id = '' , $class_id = '' , $section_id = '' , $subject_id = '')
    {
        ...
        redirect(base_url().'admin/marks_manage_view/'.$exam_id.'/'.$class_id.'/'.$section_id.'/'.$subject_id , 'refresh');
    }
    else{
        $this->session->set_flashdata('error_message' , get_phrase('select_all_the_fields'));
        $page_data['page_name']  =   'marks_manage';
    ...
    function class_routine($param1 = '', $param2 = '', $param3 = '')
    {
       ...
       /*     // 12 AM for starting time
            ...
            $data['year']           = $this->db->get_where('settings' , array('type' => 'running_year'))->row()->description;*/
            $data['class_started'] = $this->input->post('starting_date');
            $data['class_started'] = date('Y-m-d H:i:s', strtotime($data['class_started']));

            $date['class_ended'] = $this->input->post('ending_date');
            $data['class_ended'] = date('Y-m-d H:i:s', strtotime($date['class_ended']));
            ...
            $array = array(
               ...
               'day'           => '',
               'year'          => '',
               'class_started' => $data['class_started'],
               'class_ended'   => $data['class_ended']
            );
    ...
    function attendance_selector()
    {
        ...
        redirect(base_url().'admin/manage_attendance_view/'.$data['class_id'].'/'.$data['section_id'].'/'.$data['timestamp'],'refresh');
    }
    ...
    function attendance_update($class_id = '' , $section_id = '' , $timestamp = '')
    {
        ...
        redirect(base_url().'admin/manage_attendance_view/'.$class_id.'/'.$section_id.'/'.$timestamp , 'refresh');
    }
	...
	function manage_attendance2($date='',$month='',$year='',$class_id='' , $section_id = '' , $session = '')
	{
		...
		if($_POST)
		{
			...
			$this->session->set_flashdata('flash_message' , get_phrase('data_updated'));
			redirect(base_url() . 'admin/manage_attendance/'.$date.'/'.$month.'/'.$year.'/'.$class_id.'/'.$section_id.'/'.$session , 'refresh');
		}
        $page_data['date']       =	$date;
    ...
    {   if($this->input->post('class_id') == '' || $this->input->post('sessional_year') == '') {
            ...
            redirect(base_url() . 'admin/attendance_report', 'refresh');
        }
        $data['class_id']       = $this->input->post('class_id');
        $data['section_id']     = $this->input->post('section_id');
        $data['month']          = $this->input->post('month');
        $data['sessional_year'] = $this->input->post('sessional_year');
        redirect(base_url() . 'admin/attendance_report_view/' . $data['class_id'] . '/' . $data['section_id'] . '/' . $data['month'] . '/' . $data['sessional_year'], 'refresh');
    }
    ...
    function invoice($param1 = '', $param2 = '', $param3 = '')
    {
        ...
        if ($param1 == 'do_update') {
            ...
            $this->session->set_flashdata('flash_message' , get_phrase('data_updated'));
            redirect(base_url() . 'admin/income', 'refresh');
        } else if ($param1 == 'edit') {
    ...
    function group_message($param1 = "group_message_home", $param2 = ""){
      ...
      elseif ($param1 == 'group_message_read') {
        ...
        if ($_FILES['attached_file_on_messaging']['name'] != "") {
          if($_FILES['attached_file_on_messaging']['size'] > $max_size){
            $this->session->set_flashdata('error_message' , get_phrase('file_size_can_not_be_larger_that_2_Megabyte'));
            redirect(base_url() . 'admin/group_message/group_message_read/' . $param2, 'refresh');
          }
          else{
    ...
    function frontend_events($param1 = '', $param2 = '') {
      if ($param1 == 'add_event') {
        ...
        redirect(base_url() . 'admin/frontend_pages/events', 'refresh');
      }
    ...
    function frontend_gallery($param1 = '', $param2 = '', $param3 = '') {
      if ($param1 == 'add_gallery') {
        ...
        redirect(base_url() . 'admin/frontend_pages/gallery', 'refresh');
      }
      ...
      if ($param1 == 'upload_images') {
        ...
        redirect(base_url().'admin/frontend_pages/gallery_image/'.$param2, 'refresh');
      }
      if ($param1 == 'delete_image') {
        ...
        redirect(base_url().'admin/frontend_pages/gallery_image/'.$param3, 'refresh');
      }
      // if ($param1 == 'delete') {
    ...
    function frontend_news($param1 = '', $param2 = '') {
      if ($param1 == 'add_news') {
        ...
        redirect(base_url() . 'admin/frontend_pages/news', 'refresh');
      }
      if ($param1 == 'edit_news') {
    ...
    function frontend_settings($task) {
      if ($task == 'update_terms_conditions') {
        ...
        redirect(base_url() . 'admin/frontend_pages/terms_conditions', 'refresh');
      }
      if ($task == 'update_about_us') {
        ...
        redirect(base_url() . 'admin/frontend_pages/about_us', 'refresh');
      }
      if ($task == 'update_privacy_policy') {
        ...
        redirect(base_url() . 'admin/frontend_pages/privacy_policy', 'refresh');
      }
      if ($task == 'update_general_settings') {
        ...
        redirect(base_url() . 'admin/frontend_pages/general', 'refresh');
      }
      if ($task == 'update_slider_images') {
        ...
        redirect(base_url() . 'admin/frontend_pages/homepage_slider', 'refresh');
      }
    }
...
//                    $data2['roll']        = $row[1];
                    ...
                    if ($array_size == 2) {
                      ...
                      redirect(base_url() . 'admin/student_bulk_add', 'refresh');
                    }
                    elseif($array_size > 2){
    ...
    function create_room()
    {
        ...
        $page_data['page_name']  = 'create_room';
    ...
    function rooms($param1 = '', $param2 = '', $param3 = '')
    {
        ...
        if ($param1 == 'create') {
            ...
            redirect(base_url() . 'admin/create_room/', 'refresh');
        }
        if ($param1 == 'do_update') {

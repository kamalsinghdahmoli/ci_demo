...
class Parents extends CI_Controller
{
        ...
        if ($param1 == 'send_new') {
            ...
            if ($_FILES['attached_file_on_messaging']['name'] != "") {
              if($_FILES['attached_file_on_messaging']['size'] > $max_size){
                ...
                redirect(base_url() . 'parents/message/message_new/', 'refresh');
              }
              else{
    ...
    function group_message($param1 = "group_message_home", $param2 = ""){
      ...
      if ($param1 == 'group_message_read') {
        ...
        if ($_FILES['attached_file_on_messaging']['name'] != "") {
          if($_FILES['attached_file_on_messaging']['size'] > $max_size){
            $this->session->set_flashdata('error_message' , get_phrase('file_size_can_not_be_larger_that_2_Megabyte'));
            redirect(base_url() . 'parents/group_message/group_message_read/' . $param2, 'refresh');
          }
          else{
  ...
  function attendance_report_selector($student_id = "")
  {
      if($student_id != ""){
        ...
        //print_r($data);
        redirect(base_url() . 'parents/attendance_report_view/' . $data['class_id'] . '/' . $data['section_id'] . '/' . $data['month'] . '/' . $data['sessional_year'] .'/'.$data['student_id'], 'refresh');
      }
  }
